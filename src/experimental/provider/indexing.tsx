...
import {
  ...
  ChunkDimProjection,
  ChunkProjection,
  DimensionArraySelection,
  DimensionSelection,
  DimIndexer,
  Indexer,
  NormalizedArraySelection,
  Slice,
  SliceIndices,
} from "zarr/types/core/types";
...
function checkSelectionLength(
  selection: DimensionArraySelection[],
  shape: number[]
) {
 ...
 */
export function selectionToSliceIndices(
  selection: NormalizedArraySelection,
  shape: number[]
): [(number | SliceIndices)[], number[]] {
 ...
 */
export function normalizeArraySelection(
  selection: ArraySelection | number,
  shape: number[],
  convertIntegerSelectionToSlices = false
): NormalizedArraySelection {
...
export function replaceEllipsis(
  selection: ArraySelection | number,
  shape: number[]
) {
  ...
  if (numEllipsis === 1) {
    ...
    if (numItems >= shape.length) {
      ...
      let newItem = selection
        .slice(0, numItemsLeft)
        .concat(new Array(numNewItems).fill(null));
      if (numItemsRight > 0) {
        newItem = newItem.concat(
          selection.slice(selection.length - numItemsRight)
        );
      }
      selection = newItem;
    }
...
export function normalizeIntegerSelection(
  dimSelection: number,
  dimLength: number
): number {
  ...
  if (dimSelection >= dimLength || dimSelection < 0) {
    throw new BoundsCheckError(
      `index out of bounds for dimension with length ${dimLength}`
    );
  }
...
function isContiguousSlice(
  s: Slice | number | number[] | "..." | ":" | null
): boolean {
  return isSlice(s) && ((s as Slice).step === null || (s as Slice).step === 1);
}

function isPositiveSlice(
  s: Slice | number | number[] | "..." | ":" | null
): boolean {
  return (
    isSlice(s) &&
    ((s as Slice).step === null || ((s as Slice).step as number) >= 1)
  );
}
  ...
  constructor(selection: ArraySelection, array: ZarrArray) {
    ...
    for (let i = 0; i < arrayShape.length; i++) {
      ...
      if (isInteger(dimSelection)) {
        this.dimIndexers.push(
          new IntDimIndexer(dimSelection as number, dimLength, dimChunkLength)
        );
      } else if (isSlice(dimSelection)) {
        this.dimIndexers.push(
          new SliceDimIndexer(dimSelection as Slice, dimLength, dimChunkLength)
        );
      } else {
        throw new RangeError(
          `Unspported selection item for basic indexing; expected integer or slice, got ${dimSelection}`
        );
      }
    }
...
class SliceDimIndexer implements DimIndexer {
  ...
  *iter() {
    ...
    for (
      let dimChunkIndex = dimChunkIndexFrom;
      dimChunkIndex < dimChunkIndexTo;
      dimChunkIndex++
    ) {
      ...
      const dimLimit = Math.min(
        this.dimLength,
        (dimChunkIndex + 1) * this.dimChunkLength
      );
      ...
      const dimChunkSelection = slice(
        dimChunkSelStart,
        dimChunkSelStop,
        this.step
      );
      const dimChunkNumItems = Math.ceil(
        (dimChunkSelStop - dimChunkSelStart) / this.step
      );
      const dimOutSelection = slice(
        dimOutOffset,
        dimOutOffset + dimChunkNumItems
      );
      yield {
        dimChunkIndex,
        dimChunkSelection,
