...
import { ChildPortFragment, ChildPortNestedFragment, PortKind } from "../../../api/graphql";
...
const portToLabel = (port: ChildPortFragment | ChildPortNestedFragment): string => {
  if (port.kind == PortKind.Structure) return  port.identifier || "Unkonwn Structure";
  if (port.kind == PortKind.List) return  port.child ? ("List of " + portToLabel(port?.child) || "Unknown List") : "Unknown List";
  if (port.kind == PortKind.Union) return  "Union of " + port?.variants?.filter(notEmpty).map(x => x ?  portToLabel(x): "Unkown" ).join(", ");
...
export const RenderedSubWidget = ({variant, portKey}: {variant: ChildPortFragment | null |undefined, portKey: string}) => {
...
const UnionWidget: React.FC<InputWidgetProps> = ({ port, widget }) => {
  ...
  if (port.variants.length == 1) return <>  Variants amount to one. Union makes no sense </>;
        ...
        <>
          ...
          <Formik
            ...
            >
              ...
              {port.variants && <CarouselInputField
                      label={"Choose"}
                      options={port.variants.filter(notEmpty).map((x, i) => ({ label: portToLabel(x), value: i }))}
                      name="use"
                    ...
                    />}  
                {values.use != null && <RenderedSubWidget variant={port?.variants?.at(values.use)} portKey={"value"} />}
