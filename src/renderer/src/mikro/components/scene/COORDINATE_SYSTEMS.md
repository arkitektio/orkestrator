...
`@/mikro/lib/coords/transformGraph.ts` / `platform/model/layerModel.ts` / `platform/coords/levelGeometry.ts`,
re-derive from here.
...
`@/mikro/lib/coords/transformGraph.ts` — and NEVER walks `pathToWorld`
...
`mikro/components/registration`) may draw layers somewhere other than
...
- AUTHORING evaluates exactly ONE edge — the final step of `pathToWorld` — in
  its full named-axis space (`@/mikro/lib/coords/namedAffine.ts`), and
...
GraphQL documents: `graphql/mikro/fragments/{scene,lens,layer,
...
One rule of altitude: **everything below `@/mikro/lib/coords/transformGraph.ts` and
...
| --- | --- | --- | --- |
| `LayerState.affineMatrix` (voxel→world 4×4, x/y/z rows) | local prefix (`lens.toParent`, level-0 `toParent`) ∘ `pathToWorld` steps | `composeLayerAffine` (`@/mikro/lib/coords/transformGraph.ts`), once per scene load in `normalizeLayer` | `worldTransform.buildAffineMatrix` (the rendered frame — corner-anchored, §0), `nodePlanning` 2D slab inverse, `visibility`, `RoiDrawer` |
...
| Concern | Tests |
| --- | --- |
| Edge evaluation, `invert4`, placement-path composition (incl. inverted steps), layer prefix detection, unregistered degradation | `@/mikro/lib/coords/transformGraph.test.ts` |
| Pixel-factor and legacy physical-scale level edges, identity/translation edges, fallback | `@/mikro/lib/coords/transformGraph.test.ts` ("level scale factors") |
| Named-axis edge evaluation (all axes, not just spatial), world-delta × edge forward and inverted, axis extension, round-trip through `placementToSpatialAffine` | `@/mikro/lib/coords/namedAffine.test.ts` |
...
| Registration save plan (update / replace / refuse), edge eligibility, landmark solvers, gizmo drag math | `@/mikro/components/registration/math/*.test.ts` |
