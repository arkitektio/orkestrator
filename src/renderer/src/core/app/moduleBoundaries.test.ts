 ...
 * where area is the folder under `core/` (`host:app`, `host:components`) or,
 * under `core/lib/`, the lib package (`host:lib/export`). Entry files at the
 * source root (`main.tsx`, `App.tsx`) are `host:root`.
 */
...
// __dirname is src/renderer/src/core/app.
...
describe("the boundary parser", () => {
  ...
  it("owns paths by their longest root", () => {
    ...
    expect(ownerOf("core/app/dialog.tsx")).toBe("host:app");
