...
export const CreateSampleModal = modalfy(
  ({
    ...
    const { data: exp_query } = withMikro(useMyExperimentsQuery)();
    ...
    return (
      <Formik<CreateSampleMutationVariables>
        ...
        onSubmit={(values, { setSubmitting }) => {
          console.log(values);
          setSubmitting(true);
          createSample({ variables: values }).then((ex) => {
            console.log(ex);
            setShow(false);
          });
        }}
                ...
                <div className="sm:flex sm:items-start w-full">
                  <div className="mt-1 test-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    ...
                    <div className="mt-2 align-left text-left ">
                      ...
                      />
                      {exp_query?.myexperiments && (
                        <SelectInputField
                          name={"experiments"}
                          label="Experiments"
                          isMulti={true}
                          options={exp_query?.myexperiments?.map((ex) => ({
                            label: ex?.name,
                            value: ex?.id,
                          }))}
